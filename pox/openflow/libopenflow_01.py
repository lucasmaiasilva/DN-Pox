...
class ofp_match (ofp_base):
  ...
  def pack (self, flow_mod=False):
    ...
    if(self.dn_src==0):
        self.dn_src=_PAD*32
    packed += str(self.dn_src)
    packed += (32 - len(str(self.dn_src)))*_PAD


    if self.dn_dst==0:
        self.dn_dst=_PAD*32
    packed += str(self.dn_dst)
    packed += (32 - len(str(self.dn_dst)))*_PAD

    #packed += hex(self.dn_dst)

    #packed += str(self.dn_dst).encode('hex')
    #packed += (32 - len(self.dn_dst))*_PAD
    #packed += _PAD*64
    #packed += bin(self.dn_dst)
    #str(self.dn_dst).encode('hex')
    #print("Imprimindo self.dn_src=",self.dn_src,str(self.dn_src),str(self.dn_src).encode('hex'),len(str(self.dn_src).encode('hex')),sys.getsizeof(self.dn_src))
    #print("Imprimindo self.dn_dst=",self.dn_dst,str(self.dn_dst),str(self.dn_dst).encode('hex'),len(str(self.dn_dst).encode('hex')))

    #print("ofp_match",packed.encode('hex'),len(packed))
    return packed
  ...
  def show (self, prefix=''):
    ...
    outstr += append('dn_src')
    outstr += append('dn_dst')
...
ofp_match_data = {
  ...
  'dn_src' : (0,OFPFW_DN_SRC),
  'dn_dst' : (0,OFPFW_DN_DST),
}
